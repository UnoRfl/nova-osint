...
Produces `dist/NOVA.exe` — Python, Tk and the site catalogue in one file, no
install required on the target machine. The spec excludes the scientific stack
and pins the data files; the icon is generated by `python assets/make_icon.py`.
